...
class DriftDashboard:
    ...
    def _resample_particles(self):
        ...
        # Staged particle releases: initial dump 50% at t=0, remaining over first 30 steps
        total = self.n_particles
        initial_count = total // 2
        remaining_count = total - initial_count
        n_release_steps = min(30, max(1, self.max_time_steps - 1))
        per_step = remaining_count // n_release_steps
        extra = remaining_count % n_release_steps

        idx = 0
        # Initial dump at t=0
        for _ in range(initial_count):
            self.drift_objects.add_object(float(lon[idx]), float(lat[idx]), created_time_idx=0)
            idx += 1

        # Subsequent dumps over first n_release_steps time steps
        for step in range(1, n_release_steps + 1):
    ...
    def _update_display(self):
        ...
        if self.trajectories is not None:
            ...
            # Plot initial positions on left panel
            self.ax_traj.scatter(self.trajectories[0, :, 0], self.trajectories[0, :, 1], 
                                 c='green', s=20, marker='x', linewidth=1.5, zorder=4, label='Start')
