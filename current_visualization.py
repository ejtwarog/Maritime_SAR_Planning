...
def create_slider_visualization(nc_file, output_file='surface_currents.html'):
    """Create an interactive time-slider visualization of surface currents colored by magnitude."""
    ...
    for t in range(n_times):
        ...
        # Add scatter trace colored by speed magnitude
        fig.add_trace(go.Scatter(
            x=lon_sub,
            y=lat_sub,
            mode='markers',
            marker=dict(
                ...
                color=speed_sub,
                colorscale='Viridis',
                showscale=(t == 0),  # Only show colorbar for first trace
                colorbar=dict(title="Speed (m/s)"),
                line=dict(width=0),
    ...
    fig.update_layout(
        title=f"Surface Currents - Time: {data['time'][0]}",
        xaxis_title="Longitude",
        yaxis_title="Latitude",
        hovermode='closest',
        width=1200,
